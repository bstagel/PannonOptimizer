...
Dual_Ratiotest::Dual_Ratiotest(const Model & m_model)
    :m_model(m_model)
{
    m_incomingVariableIndex=0;
}

void Dual_Ratiotest::Shift(std::vector<BreakPoints>* breakpts, unsigned int startId, unsigned int stopId){
...
void Dual_Ratiotest::getNextElement(std::vector<BreakPoints>* breakpts, unsigned int length){
    BreakPoints temp;
...
void Dual_Ratiotest::PerformRatioTestPhase1(unsigned int outgoing,
                                            Vector alpha,
                                            Vector reducedCosts,
...
//computing ratios
        ...
        currentRatio.functionValue=(*phaseIObjectiveValue);
...
//dual steplength, incoming variable
    ...
    while(functionSlope>0 && iterationCounter<length){
        ...
        (*phaseIObjectiveValue)+=functionSlope*(breakpoints[length-1-iterationCounter].value-breakpoints[length-iterationCounter].value);
        breakpoints[length-1-iterationCounter].functionValue=(*phaseIObjectiveValue);
    ...
    typeOfIthVariable=m_model.getVariable(outgoing).getType();

//determining primal steplength
    Numerical::Double valueOfOutgoingVariable=variableStates.getAttachedData(outgoing);
    if(typeOfIthVariable==Variable::FIXED){
        m_primalSteplength=(valueOfOutgoingVariable-m_model.getVariable(outgoing).getLowerBound())/alpha[outgoing];
    }
        ...
        m_primalSteplength=valueOfOutgoingVariable/alpha[outgoing];
    }
    else if(typeOfIthVariable==Variable::MINUS){
        m_primalSteplength=(valueOfOutgoingVariable-m_model.getVariable(outgoing).getUpperBound())/alpha[outgoing];
    }
...
//numerical threshold
    ...
    Numerical::Double prevObjValue=*phaseIObjectiveValue, nextObjValue=*phaseIObjectiveValue;
...
void Dual_Ratiotest::PerformRatiotestPhase2(unsigned int outgoing,
                                            Vector alpha,
                                            Vector reducedCosts,
    ...
    Numerical::Double functionValue=objVal;
    ...
    breakpoints.push_back(currentRatio);
//determining t>0 or t<0 cases
    if( variableStates.getAttachedData(outgoing) < m_model.getVariable(outgoing).getLowerBound()){
        tPositive=true;
    }else if(variableStates.getAttachedData(outgoing) > m_model.getVariable(outgoing).getUpperBound()){
        tPositive=false;
    }
...
//computing ratios
    ...
    if(tPositive){
        ...
        functionSlope=Numerical::fabs(variableStates.getAttachedData(outgoing));
    //t<=0 case
    }else{
        while(!transform && it!=endit){
            variableIndex=it.getData();
        ...
        functionSlope=variableStates.getAttachedData(outgoing)-m_model.getVariable(outgoing).getUpperBound();
        }
...
//determining primal,dual steplength incoming variable
    if(!transform){
        ...
        while(!transform && functionSlope>0 && iterationCounter<length){
            ...
            functionValue+=functionSlope*(breakpoints[id].value-breakpoints[id+1].value);
            breakpoints[id].functionValue=functionValue;
...
//numerical threshold
    ...
    //if there are further breakpoints
            if(iterationCounter<length){
                ...
                iterationCounter++;
                getNextElement(&breakpoints,length-iterationCounter);
                nextObjValue=functionValue+functionSlope*(breakpoints[alphaId-2].value-breakpoints[alphaId-1].value);
                functionValue=nextObjValue>prevObjValue ? nextObjValue : prevObjValue;
